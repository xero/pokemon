...
- **Card headings**: `### Name — Set Words 056 · Reg I`. The regex wants `Name — SetWords NNN`; a matching row in `cards.csv` auto-renders the scan, stat table, and legality badge. No row, no card block, silently. Add the card to the pipeline first. Basic energy needs the zero-padded number in the heading (`Mega Evolution Energies 005`); the regex refuses one digit.
