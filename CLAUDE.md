...
- **`PAGES` is ordered and grouped, not a flat list.** The library pages (collection, wishlist, rules) come first with no heading. `## League Decks` holds the two decks we are sleeving now, and they are the only titles that carry our names ("Fox's", "Xero's"); every other deck's `# Title` drops the name. A bare string in `PAGES` is a heading with markdown hashes (`"## League Decks"`, `"### Lanterns"`), and each page after it takes the next heading level down for its title. Every card above the `OTHER` heading (`## Other Decks`) gets `data-featured`, which the template tints, so the highlight follows the section, not a row count. Retiring a league deck means moving its row under an Other Decks group and taking the name off its title.
...
- **`wishlist.html` is the pull list, and it is buy blocks only.** A row exists when a deck's ```buy block asks for more than `cards.csv` owns. Need is one deck per player: the largest ask among Xero's decks plus the largest among Fox's, because each of us sleeves one deck at a time but both get sleeved on the same night. Which decks are Fox's is the `FOX` set in `build_wishlist.py`; a new Fox deck with a buy block goes in it, or its asks share copies with Xero's decks. `quantity` 0 is deliberately *not* a source: most of those rows are a shortlist, not a shopping list — one energy-acceleration survey seeded 35 candidates at once. The reason any card entered the pipeline is the note column of `wanted-cards.tsv`, and nothing carries it into `cards.csv`.
